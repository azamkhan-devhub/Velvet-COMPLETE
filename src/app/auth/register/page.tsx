...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      toast.success('Account created! Welcome to VELVET.');
      router.push('/account');
    } catch (err: any) {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      toast.success('Welcome to VELVET!');
      router.push('/account');
    ...
    <div className="min-h-screen flex">
      <div className="flex-1 flex flex-col justify-center px-6 py-12 lg:px-16 xl:px-24">
        <div className="max-w-md w-full mx-auto">
          ...
          <p className="text-muted text-sm mb-8">Join the VELVET inner circle</p>

          <button onClick={handleGoogle} disabled={googleLoading}
            className="w-full flex items-center justify-center gap-3 border border-border py-3.5 text-sm font-body hover:bg-cream transition-colors disabled:opacity-60 mb-6">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input {...register('name')} type="text" placeholder="Fatima Khan"
            ...
            <div>
              <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Password</label>
              <div className="relative">
                <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted" />
                <input {...register('password')} type={showPwd ? 'text' : 'password'} placeholder="Min 8 chars, 1 uppercase, 1 number"
                  className={`input-field pl-10 pr-10 ${errors.password ? 'border-red-400' : ''}`} />
                <button type="button" onClick={() => setShowPwd(!showPwd)}
                  className="absolute right-3.5 top-1/2 -translate-y-1/2 text-muted hover:text-black">
            ...
            <div>
              <label className="block text-[10px] tracking-[0.15em] uppercase text-muted mb-1.5">Confirm Password</label>
              <div className="relative">
                <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted" />
            ...
            <p className="text-[11px] text-muted">By creating an account you agree to our <Link href="/terms" className="underline">Terms</Link> and <Link href="/privacy" className="underline">Privacy Policy</Link>.</p>
          ...
          <p className="text-center text-sm text-muted mt-6">
            ...
            <Link href="/auth/login" className="text-black hover:text-gold font-medium">Sign in</Link>
          </p>
        </div>
      </div>
      ...
      <div className="hidden lg:block flex-1 relative bg-black">
        <div className="absolute inset-0 bg-cover bg-center opacity-60"
          style={{ backgroundImage: "url('https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=1200&q=85')" }} />
        <div className="absolute inset-0 bg-gradient-to-r from-black/50 to-transparent" />
        <div className="relative h-full flex flex-col justify-end p-16">
          <p className="font-display text-5xl font-light text-white italic leading-tight">&ldquo;Invest in quality.<br />It never goes out of style.&rdquo;</p>
        </div>
      </div>
    </div>
  );
